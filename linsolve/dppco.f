      ...
      use type_module
      IMPLICIT   none
      integer(kind=i4_kind)  n,info
      real(kind=r8_kind) ap(*),z(*)
      real(kind=r8_kind) rcond
c
...
c
      real(kind=r8_kind) ddot,ek,t,wk,wkm
      real(kind=r8_kind) anorm,s,dasum,sm,ynorm
      integer(kind=i4_kind) i,ij,j,jm1,j1,k,kb,kj,kk,kp1
      intrinsic dmax1,dabs,dsign
c
c
c     find norm of a
c
...
c
   ...
   50    continue
         ...
         do 110 k = 1, n
            kk = kk + k
            if (z(k) .ne. 0.0_r8_kind) ek = dsign(ek,-z(k))
...
c        make znorm = 1.0
         ...
         ynorm = s*ynorm
c
         if (anorm .ne. 0.0_r8_kind) rcond = ynorm/anorm
         if (anorm .eq. 0.0_r8_kind) rcond = 0.0_r8_kind
  180 continue
      return
      end
