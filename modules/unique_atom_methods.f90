...
module unique_atom_methods
  ...
  !
  !
  !  Purpose: Database for Coordinates and Coefficients of primitive,
  !           contracted and symmetry adapted basis functions.
  !           Subroutines for reading and writing this database and
  !           packing and unpacking it are also defined.  Memory
  !           allocation is automatically done by these subroutines.
  !           Subroutines doing memory allocation are made public for
  !           data that will be calculated externally.
  !
  !  Due to possible needs to replace parts of reading by internal
  !  and possibly parallel calculations, the reading, writing, packing
  !  and unpacking of data was divided over several routines that have to be
  !  called in the following sequence (replace xxx by read, write, pack,
  !  or unpack):
  !      unique_atom_xxx
  !      or for new symetry part: unique_atom_unique_xxx (only read/write)
  !      unique_atom_xxx_basis (only read/write)
  !      unique_atom_glob_cons_xxx (only pack and unpack)
  !      unique_atom_symadapt_xxx
  !      unique_atom_symequiv_xxx
  !       (the number of irreps and partners in symmetry_data_module required)
  !      unique_atom_renorm_xxx
  !       (the number of irreps and partners in symmetry_data_module required)
  !  for packing and unpacking additionally in end:
  !      unique_atom_localvar_xxx
  !
  !
  !  unique_atom_setup() must be called after symmetry part
  !  or reading in input to calculate some quantities
  !  needed in integlral and scf-part !!
  !
  !
  !
...
!     endif
   ...
   !*************************************************************
   subroutine unique_atom_symequiv_pack()
     ...
     use commpack_module
     !------------ Declaration of local variables -----------------
     integer(kind=i4_kind) :: ia,ib,status,non_dist
     !------------ Declaration of subroutines used ----------------
     external error_handler
     !------------ Executable code --------------------------------

     !
     ! Packs the lower triangle of unique_atom_symequiv(:, :)
     !
     do ia=1,N_unique_atoms
        do ib=1,ia

           non_dist = unique_atom_symequiv(ia,ib)%n_equiv_dist
           call commpack(non_dist,status)
           if(status.ne.0) call error_handler &
                ("unique_atom_symequiv_pack : pack failed")
           call commpack &
                (unique_atom_symequiv(ia,ib)%index_eq_atom2, &
                non_dist,1,status)
           if(status.ne.0) call error_handler &
                ("unique_atom_symequiv_pack : pack 2 failed")
           call commpack &
                (unique_atom_symequiv(ia,ib)%weight,&
                non_dist,1,status)
           if(status.ne.0) call error_handler &
                ("unique_atom_symequiv_pack : pack 3 failed")

        enddo
     enddo

   end subroutine unique_atom_symequiv_pack
   !*************************************************************


   !*************************************************************
   subroutine unique_atom_symequiv_unpack()
     ! Purpose: unpacks all the information contained
     !          in unique_atom_symequiv(:,:)
     !** End of interface *****************************************
     !------------ modules used ------------------- ---------------
     use commpack_module
     !------------ Declaration of local variables -----------------
     ...
     allocate(unique_atom_symequiv(N_unique_atoms,N_unique_atoms), &
          STAT=status)
     if(status.ne.0) call error_handler &
          ("unique_atom_symequiv_unpack : allocate 1 failed")

     do ia=1,N_unique_atoms
        do ib=1,ia

           call communpack(unique_atom_symequiv(ia,ib)%n_equiv_dist, &
                status)
           if(status.ne.0 ) call error_handler &
                ("unique_atom_symequiv_unpack : unpack 1 failed")
           non_dist = unique_atom_symequiv(ia,ib)%n_equiv_dist
           allocate &
                (unique_atom_symequiv(ia,ib)%index_eq_atom2(non_dist),&
                STAT=status)
           if(status.ne.0) call error_handler &
                ("unique_atom_symequiv_unpack : allocate 2 failed")
           allocate &
                (unique_atom_symequiv(ia,ib)%weight(non_dist), &
                STAT=status)
           if(status.ne.0) call error_handler &
                ("unique_atom_symequiv_unpack : allocate 3 failed")

           ! FIXME: the upper triangle is left unallocated, with arbitrary
           !        data:
           if (ia.ne.ib) then
              ! These are left unallocated:
              ! unique_atom_symequiv(ib,ia)%index_eq_atom2
              ! unique_atom_symequiv(ib,ia)%weight
              unique_atom_symequiv(ib,ia)%n_equiv_dist = 0
           endif

           call communpack(unique_atom_symequiv(ia,ib)%index_eq_atom2,&
                non_dist,1,status)
           if(status.ne.0 ) call error_handler &
                ("unique_atom_symequiv_unpack : unpack 2 failed")
           call communpack(unique_atom_symequiv(ia,ib)%weight,&
                non_dist,1,status)
           if(status.ne.0 ) call error_handler &
                ("unique_atom_symequiv_unpack : unpack 3 failed")

        enddo
     enddo

   end subroutine unique_atom_symequiv_unpack
   !*************************************************************
  ...
  subroutine unique_atom_bcast()
    ...
    if ( comm_rank() == 0 ) then
        ...
        call unique_atom_pack()
        call comm_send()
    else
        call comm_save_recv(comm_master_host, msgtag_packed_message)
        call unique_atom_unpack()
    endif
  end subroutine unique_atom_bcast

   !*************************************************************
   subroutine unique_atom_pack()
   !  Purpose: packs all information contained in
   !           unique_atoms(:)
   !           exception: information about symmetry adaption
   !** End of interface *****************************************
   !------------ modules used ------------------- ---------------
   use commpack_module
   use xpack
   ...
   !------------ Declaration of local variables -----------------
   type(unique_atom_type), pointer :: ua
   integer             ::  i,j,status,length
   logical             ::  pseudopot, core_dens
   !------------ Declaration of subroutines used ----------------
   external error_handler
   !------------ Executable code --------------------------------
   call commpack(N_unique_atoms,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_pack: error at N_unique_atoms.")
   call commpack(N_moving_unique_atoms,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_pack: error at N_moving_unique_atoms.")
   ...
   do i=1,N_unique_atoms
      ...
      call commpack(ua%Z,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_pack: error at Z")
      call commpack(ua%ZC,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_pack: error at ZC")
      call commpack(ua%N_equal_atoms,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_pack: error at N_equal_atoms")
      call commpack(ua%moving_atom,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_pack: error at moving_atom")
      call commpack(ua%lmax_ob,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_pack: error at lmax_ob")
      if (pseudopot) then
         call commpack(ua%lmax_pseudo,status)
         if (status .ne. 0) call error_handler( &
              "unique_atom_pack: error at lmax_pseudo")
      endif
      call commpack(ua%lmax_ch,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_pack: error at lmax_ch")
      call commpack(ua%lmax_xc,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_pack: error at lmax_xc")
      call commpack(ua%N_glob_cons_ch,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_pack: error at N_glob_cons_ch")
      call commpack(ua%N_glob_cons_xc,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_pack: error at N_glob_cons_xc")
      call commpack(ua%name,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_pack: error at name")

      call pck(ua%nuclear_spin)
      call pck(ua%nuclear_magnetic_moment)
      call pck(ua%nuclear_radius)

      ! pack atom coordinates of all equal atoms
      length = ua%N_equal_atoms * 3
      call commpack(ua%position(1,1), length, 1, status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_pack: error at positions.")

      ! pack l dependent basis information for orbitals
      do j=0,ua%lmax_ob
         call unique_atom_basis_pack(ua%l_ob(j))
      enddo

      if (pseudopot) then
         do j=0,ua%lmax_pseudo
             call unique_atom_pseudopot_pack(ua%l_pseudopot(j))
         enddo
      endif

#ifdef WITH_CORE_DENS
      ! pack atomic density fitting basis functions for r2_type
      call unique_atom_atomic_dens_pack(ua%r2_core)
      ! pack atomic density fitting basis functions for s_type
      call unique_atom_atomic_dens_pack(ua%s_core)
#endif

      ! read r**2 basis information for charge fitfcts
      call unique_atom_basis_pack(ua%r2_ch)

      ! pack l dependent basis information for charge fitfcts
      do j=0,ua%lmax_ch
         call unique_atom_basis_pack(ua%l_ch(j))
      enddo

      ! pack r**2 basis information for exchange fitfcts
      call unique_atom_basis_pack(ua%r2_xc)

      ! pack l dependent basis information for exchange fitfcts
      do j=0,ua%lmax_xc
         call unique_atom_basis_pack(ua%l_xc(j))
      enddo

   enddo

     ! pack information about unique atom global contractions
     call unique_atom_glob_cons_pack()

     ! pack information about symmetry adaption of unique atoms
     call unique_atom_symadapt_pack()

     ! pack information about different sym.equiv atom pairs
     call unique_atom_symequiv_pack()

     ! pack local variables of unique_atom_module
     call unique_atom_localvar_pack()
   end subroutine unique_atom_pack
   !*************************************************************


   !*************************************************************
   subroutine unique_atom_unpack()
   !  Purpose: unpacks all information contained in
   !           unique_atoms(:) and does allocation
   !           exception: information about symmetry adaption
   !** End of interface *****************************************
   !------------ modules used ------------------- ---------------
   use commpack_module
   ...
   external error_handler
   !------------ Executable code --------------------------------
   call communpack(N_unique_atoms,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_unpack: error at N_unique_atoms.")
   call communpack(N_moving_unique_atoms,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_unpack: error at N_moving_unique_atoms.")

   allocate( unique_atoms(N_unique_atoms), stat=status )
   if (status .ne. 0) call error_handler( &
        "unique_atom_unpack: allocate failed.")
   ...
   do i=1,N_unique_atoms
      ...
      call communpack(ua%Z,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_unpack: error at Z")
      call communpack(ua%ZC,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_unpack: error at ZC")

      pseudopot = ua%zc /= 0.0_r8_kind .and. .not.operations_core_density
      core_dens = ua%zc /= 0.0_r8_kind .and.      operations_core_density

      call communpack(N_equal_atoms,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_unpack: error at N_equal_atoms")
      call communpack(ua%moving_atom,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_unpack: error at moving_atom")
      call communpack(lmax_ob,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_unpack: error at lmax_ob")
      if (pseudopot) then
         call communpack(lmax_pseudo,status)
         if (status .ne. 0) call error_handler( &
              "unique_atom_unpack: error at lmax_pseudo")
      endif
      call communpack(lmax_ch,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_unpack: error at lmax_ch")
      call communpack(lmax_xc,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_unpack: error at lmax_xc")
      call communpack(ua%N_glob_cons_ch,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_unpack: error at N_glob_cons_ch")
      call communpack(ua%N_glob_cons_xc,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_unpack: error at N_glob_cons_xc")
      call communpack(ua%name,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_unpack: error at name")

      call upck(ua%nuclear_spin)
      call upck(ua%nuclear_magnetic_moment)
      call upck(ua%nuclear_radius)

      ua%lmax_all = 0
      if (operations_gradients .and. ua%moving_atom > 0) ua%lmax_all = 1

      call unique_atom_alloc(ua, lmax_ob, lmax_ch, lmax_xc, N_equal_atoms )
      ...
      length = ua%N_equal_atoms * 3
      call communpack(ua%position(1,1), length, 1, status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_unpack: error at positions.")
      ! and restore position_first_ea
      ua%position_first_ea = ua%position(:,1)

      ! read l dependent basis information for orbitals
      do j=0,lmax_ob
         call unique_atom_basis_unpack(ua%l_ob(j))
      enddo

      if (pseudopot) then
         do j=0,lmax_pseudo
             call unique_atom_pseudopot_unpack(ua%l_pseudopot(j))
         enddo
      endif

#ifdef WITH_CORE_DENS
      ! unpack atomic density fitting basis functions for r2_core
      call unique_atom_atomic_dens_unpack(ua%r2_core)
      ! unpack atomic density fitting basis functions for s_core
      call unique_atom_atomic_dens_unpack(ua%s_core)
#endif

      ! read r**2 basis information for charge fitfcts
      call unique_atom_basis_unpack(ua%r2_ch)

      ! read l dependent basis information for charge fitfcts
      do j=0,lmax_ch
         call unique_atom_basis_unpack(ua%l_ch(j))
      enddo

      ! read r**2 basis information for exchange fitfcts
      call unique_atom_basis_unpack(ua%r2_xc)

      ! read l dependent basis information for exchange fitfcts
      do j=0,lmax_xc
         call unique_atom_basis_unpack(ua%l_xc(j))
      enddo
   enddo

     ! unpack information about unique atom global contractions
     call unique_atom_glob_cons_unpack()

     ! unpack information about symmetry adaption of unique atoms
     call unique_atom_symadapt_unpack()

     ! unpack information about different sym.equiv atom pairs
     call unique_atom_symequiv_unpack()

     ! pack local variables of unique_atom_module
     call unique_atom_localvar_unpack()
   end subroutine unique_atom_unpack
   !*************************************************************
   ...
   !*************************************************************
   subroutine unique_atom_symadapt_pack()
     use uatom_symmadapt, only: ua_pack_everything
     implicit none
     ! *** end of interface ***

     call ua_pack_everything()
   end subroutine unique_atom_symadapt_pack

   subroutine unique_atom_symadapt_unpack()
     use uatom_symmadapt, only: ua_unpack_everything
     implicit none
     ! *** end of interface ***

     call ua_unpack_everything()
   end subroutine unique_atom_symadapt_unpack
   !*************************************************************
   ...
   !*************************************************************
   subroutine unique_atom_basis_pack(l)
   ...
   ! pack dimensions
   call commpack(l%N_exponents, status)
   if (status .ne. 0) call error_handler( &
        "unique_atom_basis_pack: error at N_exponents")
   call commpack(l%N_uncontracted_fcts, status)
   if (status .ne. 0) call error_handler( &
        "unique_atom_basis_pack: error at N_uncontracted_fcts")
   call commpack(l%N_contracted_fcts, status)
   if (status .ne. 0) call error_handler( &
        "unique_atom_basis_pack: error at N_contracted_fcts")
   call pck(size(l%functions, 2))

   ! pack exponents
   call commpack(l%exponents, l%N_exponents, 1, status)
   if (status .ne. 0) call error_handler( &
        "unique_atom_basis_pack: error at exponents")

   ! pack norms
   call commpack(l%norms, size(l%norms), 1, status)
   if (status .ne. 0) call error_handler( &
        "unique_atom_basis_pack: error at norms")

   ! pack contractions
   do i = 1,l%N_contracted_fcts
      do j = 1,l%N_exponents
         call commpack(l%contractions(j,i), status)
         if (status .ne. 0) call error_handler( &
              "unique_atom_basis_pack: error at contractions")
      enddo
   enddo

   ! pack linear combinaitons
   call pck(l%functions)
   end subroutine unique_atom_basis_pack
   !*************************************************************


   !*************************************************************
   subroutine unique_atom_basis_unpack(l)
   ...
   call communpack(N_exponents,status)
   if (status .ne. 0) call error_handler( &
        "unique_atom_basis_unpack: error at N_exponents")
   call communpack(N_uncontracted_fcts,status)
   if (status .ne. 0) call error_handler( &
        "unique_atom_basis_unpack: error at N_uncontracted_fcts")
   call communpack(N_contracted_fcts,status)
   if (status .ne. 0) call error_handler( &
        "unique_atom_basis_unpack: error at N_contracted_fcts")
   call upck(N_fcts)

   call unique_atom_basis_alloc(l, N_exponents, &
        N_uncontracted_fcts, N_contracted_fcts, N_fcts)

   ! unpack exponents
   call communpack(l%exponents, N_exponents, 1, status)
   if (status .ne. 0) call error_handler( &
        "unique_atom_basis_unpack: error at exponents")

   ! unpack norms
   call communpack(l%norms, size(l%norms), 1, status)
   if (status .ne. 0) call error_handler( &
        "unique_atom_basis_unpack: error at norms")

   ! unpack contractions
   do i = 1,l%N_contracted_fcts
      do j = 1,l%N_exponents
         call communpack(l%contractions(j,i), status)
         if (status .ne. 0) call error_handler( &
              "unique_atom_basis_unpack: error at contractions")
      enddo
   enddo

   ! unpack linear combinations:
   call upck(l%functions)
   end subroutine unique_atom_basis_unpack
   !*************************************************************
   ...
   !*************************************************************
   subroutine unique_atom_glob_cons_pack()
   !  Purpose: packs all information about glob_cons
   !   for all unique atoms
   !------------ modules used ------------------- ---------------
   use commpack_module
   !------------ Declaration of local variables -----------------
   type(unique_atom_type), pointer :: ua
   integer             ::  i_ua,i_gc,status
   !** End of interface *****************************************
   !------------ Declaration of subroutines used ----------------
   external error_handler
   !------------ Executable code --------------------------------
   ...
   do i_ua=1,N_unique_atoms
      ...
      call commpack(ua%N_glob_cons_ch,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_glob_cons_pack: error at N_glob_cons_ch")
      call commpack(ua%N_glob_cons_xc,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_glob_cons_pack: error at N_glob_cons_xc")

      ! loop over charge contractions
      do i_gc = 1, ua%N_glob_cons_ch
         call unique_atom_glob_con_pack(ua%glob_con_ch(i_gc))
      enddo

      ! loop over exchange contractions
      do i_gc = 1, ua%N_glob_cons_xc
         call unique_atom_glob_con_pack(ua%glob_con_xc(i_gc))
      enddo

   enddo
   end subroutine unique_atom_glob_cons_pack
   !*************************************************************


   !*************************************************************
   subroutine unique_atom_glob_cons_unpack()
   !  Purpose: unpacks all information about glob_cons
   !   for all unique atoms
   !------------ modules used ------------------- ---------------
   use commpack_module
   !------------ Declaration of local variables -----------------
   ...
   do i_ua=1,N_unique_atoms
      ...
      call communpack(N_glob_cons_ch,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_glob_cons_unpack: error at N_glob_cons_ch")
      call communpack(N_glob_cons_xc,status)
      if (status .ne. 0) call error_handler( &
           "unique_atom_glob_cons_unpack: error at N_glob_cons_xc")

      call  unique_atom_glob_cons_alloc(ua,N_glob_cons_ch, N_glob_cons_xc)

      ! loop over charge contractions
      do i_gc = 1, ua%N_glob_cons_ch
         call unique_atom_glob_con_unpack(ua%glob_con_ch(i_gc))
      enddo

      ! loop over exchange contractions
      do i_gc = 1, ua%N_glob_cons_xc
         call unique_atom_glob_con_unpack(ua%glob_con_xc(i_gc))
      enddo

   enddo
   end subroutine unique_atom_glob_cons_unpack
   !*************************************************************


   !*************************************************************
   subroutine unique_atom_glob_con_unpack(c)
   !  Purpose: unpacks all information in c and does allocation
   !------------ modules used ------------------- ---------------
   use commpack_module
   !------------ Declaration of formal parameters ---------------
   type(unique_atom_glob_con_type), intent(inout)  :: c
   ...
   external error_handler
   !------------ Executable code --------------------------------
   call communpack(c%N_contributing_fcts,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_glob_con_unpack: N_contributing_fcts")
   call unique_atom_glob_con_alloc(c,c%N_contributing_fcts)
   call communpack(c%l,c%N_contributing_fcts,1,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_glob_con_unpack: l")
   call communpack(c%index_ind_fct,c%N_contributing_fcts,1,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_glob_con_unpack: index_ind_fct")
   call communpack(c%index_exp,c%N_contributing_fcts,1,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_glob_con_unpack: index_exp")
   call communpack(c%coefs,c%N_contributing_fcts,1,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_glob_con_unpack: coefs")
   end subroutine unique_atom_glob_con_unpack
   !*************************************************************


   !*************************************************************
   subroutine unique_atom_glob_con_pack(c)
   !  Purpose: packs all information in c
   !------------ modules used ------------------- ---------------
   use commpack_module
   !------------ Declaration of formal parameters ---------------
   type(unique_atom_glob_con_type), intent(in)  :: c
   ...
   external error_handler
   !------------ Executable code --------------------------------
   call commpack(c%N_contributing_fcts,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_glob_con_pack: N_contributing_fcts")
   call commpack(c%l,c%N_contributing_fcts,1,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_glob_con_pack: l")
   call commpack(c%index_ind_fct,c%N_contributing_fcts,1,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_glob_con_pack: index_ind_fct")
   call commpack(c%index_exp,c%N_contributing_fcts,1,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_glob_con_pack: index_exp")
   call commpack(c%coefs,c%N_contributing_fcts,1,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_glob_con_pack: coefs")
   end subroutine unique_atom_glob_con_pack
   !*************************************************************


   !*************************************************************
   subroutine unique_atom_localvar_pack()
   !  Purpose: packs some local variables of module
   !** End of interface *****************************************
   !------------ modules used ------------------- ---------------
   use commpack_module
   !------------ Declaration of local variables -----------------
   ...
   external error_handler
   !------------ Executable code --------------------------------
   call commpack(unique_atom_lmax_all,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_localvar_pack: error at unique_atom_lmax_all")
   call commpack(unique_atom_lmax_ob,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_localvar_pack: error at unique_atom_lmax_ob")
   call commpack(unique_atom_lmax_ch,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_localvar_pack: error at unique_atom_lmax_ch")
   call commpack(unique_atom_lmax_xc,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_localvar_pack: error at unique_atom_lmax_xc")
   call commpack(pseudopot_present,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_localvar_pack: error at pseudopot ")
   call commpack(core_density_setup,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_localvar_pack: error at core_density_setup ")
   end subroutine unique_atom_localvar_pack
   !*************************************************************


   !*************************************************************
   subroutine unique_atom_localvar_unpack()
   !  Purpose: packs some local variables of module
   !** End of interface *****************************************
   !------------ modules used ------------------- ---------------
   use commpack_module
   ...
   external error_handler
   !------------ Executable code --------------------------------
   call communpack(unique_atom_lmax_all,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_localvar_unpack: error at unique_atom_lmax_all")
   call communpack(unique_atom_lmax_ob,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_localvar_unpack: error at unique_atom_lmax_ob")
   call communpack(unique_atom_lmax_ch,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_localvar_unpack: error at unique_atom_lmax_ch")
   call communpack(unique_atom_lmax_xc,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_localvar_unpack: error at unique_atom_lmax_xc")
   call communpack(pseudopot_present,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_localvar_unpack: error at pseudopot ")
   call communpack(core_density_setup,status)
   if (status .gt. 0) call error_handler( &
        "unique_atom_localvar_unpack: error at core_density_setup ")
   end subroutine unique_atom_localvar_unpack
   !*************************************************************
  ...
  subroutine unique_atom_pseudopot_pack(l)
  !  Purpose: pack all information in l
  !--- end of interface --------------------------------------------------------
  !--- modules used ------------------------------------------------------------
  use commpack_module
  ...
  call commpack(l%n_exponents,status)
    if(status.ne.0) call error_handler(&
    "unique_atom_pseudopot_pack: error at n_exponents.")

  ! pack symmetry index of Gaussians

  call commpack(l%powers, l%n_exponents, 1, status)
    if(status.ne.0) call error_handler(&
   "unique_atom_pseudopot_pack: error at n_or_sym.")

  ! pack exponents for Gaussians

  call commpack(l%exponents, l%n_exponents, 1, status)
   if(status.ne.0) call error_handler(&
  "unique_atom_pseudopot_pack: error at cexponents.")

  ! pack coefficients for Gaussians

  call commpack(l%coefficients, l%n_exponents, 1, status)
   if(status.ne.0) call error_handler(&
  "unique_atom_pseudopot_pack: error at coefficients.")

  end subroutine unique_atom_pseudopot_pack
   !*************************************************************
  ...
  !*****************************************************************************
  subroutine unique_atom_pseudopot_unpack(l)
  ...
  call communpack(n_exponents,status)
    if(status.ne.0) call error_handler(&
    "unique_atom_pseudopot_unpack: error at n_terms.")

  call unique_atom_pseudopot_alloc(l,n_exponents)

  ! unpack symmetry index of Gaussians

  call communpack(l%powers, l%n_exponents, 1, status)
    if(status.ne.0) call error_handler(&
   "unique_atom_pseudopot_unpack: error at n_or_sym.")

  ! unpack exponents for Gaussians

  call communpack(l%exponents, l%n_exponents, 1, status)
   if(status.ne.0) call error_handler(&
  "unique_atom_pseudopot_unpack: error at cexponents.")

  ! unpack coefficients for Gaussians

  call communpack(l%coefficients, l%n_exponents, 1, status)
   if(status.ne.0) call error_handler(&
  "unique_atom_pseudopot_unpack: error at coefficients.")

  end subroutine unique_atom_pseudopot_unpack
   !*************************************************************

#ifdef WITH_CORE_DENS
  subroutine unique_atom_atomic_dens_pack(l)
  !  Purpose: pack all information in l
  !--- end of interface --------------------------------------------------------
  !--- modules used ------------------------------------------------------------
  use commpack_module
  ...
  call commpack(l%n_exponents,status)
    if(status.ne.0) call error_handler(&
    "unique_atom_atomic_dens_pack: error at n_exponents.")

!  if(l%n_exponents==-1) RETURN ! was not used
  if(l%n_exponents==0) RETURN ! was not used

  ASSERT(allocated(l%exponents))
  ASSERT(allocated(l%contractions))

  ! pack exponents for Gaussians

  call commpack(l%exponents, l%n_exponents, 1, status)
  if(status.ne.0) call error_handler(&
       "unique_atom_atomic_dens_pack: error at exponents.")

  ! pack coefficients for Gaussians
  call commpack(l%contractions, l%n_exponents, 1, status)
  if(status.ne.0) call error_handler(&
       "unique_atom_atomic_dens_pack: error at contractions.")
  end subroutine unique_atom_atomic_dens_pack

  subroutine unique_atom_atomic_dens_unpack(l)
  !  purpose : read in all information in l and dose allocation
  !--- end of interface --------------------------------------------------------
  !---------------------- modules used -----------------------------------------
  use commpack_module
  ...
  call communpack(n_exponents,status)
    if(status.ne.0) call error_handler(&
    "unique_atom_atomic_dens_unpack: error at n_terms.")

!  if(n_exponents==-1) RETURN ! is not used
  if(n_exponents==0) RETURN ! is not used
  ...
  call communpack(l%exponents, l%n_exponents, 1, status)
  if(status.ne.0) call error_handler(&
       "unique_atom_atomic_dens_unpack: error at exponents.")

  ! unpack coefficients for Gaussians

  call communpack(l%contractions, l%n_exponents, 1, status)
  if(status.ne.0) call error_handler(&
       "unique_atom_atomic_dens_unpack: error at contractions.")
  end subroutine unique_atom_atomic_dens_unpack
#endif
   !*************************************************************
