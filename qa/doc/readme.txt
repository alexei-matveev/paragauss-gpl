**********************************************
*** TRY TO STICK TO WIKI FORMATTING STYLE! ***
**********************************************

== Testsuite ==

The testsuite is used to detect regressions in the developing
program. Passing a testsuite is a prereuisite for a "public"
release.
...
The <tt>chmod</tt> commad is needed to set the executable bit on
the scripts after initial download.
...
In case there are differences (remember that real numbers could be compare only up to some precision)
you will see the output of the <tt>diff</tt> command:
 ...
 ...
 FAILED 6 of 6 tests!
and the summary at the end saying which tests and at which precision have failed.
The test fails by default if any two numbers still differ
after truncating them to 8 decimal figures.
