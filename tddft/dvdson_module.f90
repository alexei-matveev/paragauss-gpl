...
!=====================================================================
MODULE  dvdson_module
  !-------------------------------------------------------------------
  !
  !  Purpose:
  !  Contains subroutines from the F77 davidson eigensolver
  !  packet DVDSON.F
  ...
  !
  !  Main modification is that these routines no longer require
  !  an external subroutine for block matrix operations as
  !  ACTUAL PARAMETER, but call the subroutine dav_block_mult()
  !  from the "dav_block_module".
  !
  !  UPDATE: see USE eigenblock_module, only: eigenblock_mult
  !          in SUBROUTINE ADDABS(N,LIM,HIEND,KPASS,NNCV,BASIS,AB,S)
  !
  !  This construction must be used to get rid of the F77 COMMON blocks.
  !  In addition, the source code has been converted from F77 style
  !  fixed format into Fortran 90 free format.
  !
  !  Module called by: dav_solve_module
  !
  !  Author: HH
  !  Date:   8/98
  !
  ...
  !
  !-------------------------------------------------------------------
  ! here we store the matrix and do block operations
  USE type_module
...
CONTAINS
  ...
  SUBROUTINE DVDSON(N,LIM,DIAG,&
    ...
    IF ((NUME.GT.LIM).OR.((NUME.EQ.LIM).AND.(NUME.NE.N))) IERR=IERR+128
    !       ..Size of Block out of bounds
    IF ( (MBLOCK.LT.1).OR.(MBLOCK.GT.NEIG) ) IERR=IERR+256

    !       ..Check for enough workspace for Dvdson
    IF ((IWRSZ.LT.(LIM*(2*N+LIM+(NUME+10))+NUME)).OR. (IIWSZ.LT.(6*LIM+NUME))) IERR=IERR+512
    ...
    iSTART=NIV
    CALL SETUP(N,LIM,NUME,HIEND,DIAG,WORK(iscra1),& !!iscra1
  ...
  !=======================================================================
  SUBROUTINE SETUP(N,LIM,NUME,HIEND,DIAG,MINELEM,&
       BASIS,AB,S,NIV)
    !=======================================================================
    ...
    !-----------------------------------------------------------------------
    IMPLICIT DOUBLE PRECISION(A-H,O-Z)
    DIMENSION DIAG(N),BASIS(N*LIM),AB(N*LIM) !! +1??
    DIMENSION S(LIM*(LIM+1)/2)
    REAL(KIND=r8_kind), DIMENSION(LIM) :: MINELEM
    LOGICAL   HIEND
    !-----------------------------------------------------------------------
    !   on entry
    !   --------
    ...
    IF ((NIV.GT.LIM).OR.(NIV.LT.NUME)) THEN
       ...
       DO I=1,NUME
          ...
          IMIN=J
          DO J=IMIN+1,N
             IF ((AB(J).LT.0).AND.(DIAG(J).LT.DIAG(IMIN))) IMIN=J
          END DO
          MINELEM(I)=IMIN
          AB(IMIN)=1.D0
       END DO
...
#else
       CALL DCOPY(N,AB((I-1)*N+1),1,TAB,1)
       CALL DAXPY(N,-EIGVAL(I),BASIS((I-1)*N+1),1,TAB,1)
       SCRA1(I)=DDOT(N,TAB,1,TAB,1)
       SCRA1(I)=SQRT(SCRA1(I))
#endif
    END DO
...
60  RETURN
  ...
  SUBROUTINE ADDABS(N,LIM,HIEND,KPASS,NNCV,BASIS,AB,S)
    ...
    CALL eigenblock_mult(N,NNCV,BASIS(IDSTART:IDSTART+N*NNCV-1),&
         & AB(IDSTART:IDSTART+N*NNCV-1))
    !
  ...
  SUBROUTINE NEWVEC(N,NUME,LIM,MBLOCK,KPASS,CRITR,ORTHO,NNCV,INCV,&
    ...
    DO I=1,NNCV
       ...
       ELSE
          ...
          NADDED=NADDED+1
          INCV(NADDED)=INDX
          IF ((NADDED+KPASS).EQ.LIMADD) EXIT
