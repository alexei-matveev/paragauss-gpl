...
C
     ...
     *   MAXOP, MAXJ, IND(*), IERR
      DOUBLE PRECISION VEC(NMVEC,*), VAL(NMVAL,*), WORK(*)
      EXTERNAL OP, IOVECT
C
...
C
     ...
     *   NOP, IND(1), IERR
      LOGICAL RARITZ, SMALL
      DOUBLE PRECISION VAL(*), VEC(NMVEC,*), P0(N,*), P1(N,*),
     *   P2(N,*), RES(*), TAU(*), OTAU(*), T(NBAND,*),
     *   ALP(NBLOCK,*), BET(NBLOCK,*), BOUND(*), ATEMP(*),
     *   VTEMP(*), D(*), S(MAXJ,*), DELTA, EPS
      EXTERNAL OP, IOVECT
C
...
!
! FIXME: this is an unused array to fulfill the calling convention:
!
      DOUBLE PRECISION UNUS(1)
C
...
C
!
! FIXME: IFLAG=0 here tells DVSORT() not to touch the next,
!        array valued parameter. However passing a scalar,
!        where an array is expected is not quite kosher.
!        We pass an array UNUS instead:
!
!     IF(NPERM .NE. 0) CALL DVSORT(NPERM+NGOOD, VAL, RES, 0, TEMP,
      IF(NPERM .NE. 0) CALL DVSORT(NPERM+NGOOD, VAL, RES, 0, UNUS,
     *   NMVEC, N, VEC)
      NPERM = NPERM + NGOOD
      NLEFT = NLEFT - NGOOD
