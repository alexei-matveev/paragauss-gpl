...
!
     module ewald_module
	  ...
	  real(kind=r8_kind)               :: an   ! real for charge type
       ...
       integer(kind=i4_kind), parameter    :: input_unit=5, output_unit=6 ,gxc_unit=7
	...
	logical, dimension(:,:), allocatable:: type_number
     ...
     contains
       ...
       subroutine ewald
         ...
         integer(kind=i4_kind)                           :: nlm,m,l
         character(len=100)                              :: format

!         call getenv_("PWD",work_dir) undescored getenv_(...) for Linux
         call getenv("PWD",work_dir)
         input_file= trim(work_dir)//"/"//"ewald_input"
         output_file=trim(work_dir)//"/"//"ewald_output"
...
!++++++++++++++++++++++++++++ Store embedded cluster ++++++++++++++++++++++++++
	    ...
	    write(30,*) n_screep_points+n_ewald_pc,r_screep,Rc_screep
...
!
	 ...
	 real(kind=r8_kind)::an
         ...
         integer(kind=i4_kind)                         :: i_vector,i_atom,i,j,k
         integer(kind=i4_kind)                         :: alloc_stat,ioerr
         character(len=100)                            :: input_key,format,temp_string,string_read
         character(len=100)                            :: axes,units, cluster_key,pc_key, task_key
         logical                                       :: task_defined, end_input_read
...
!              
         write(output_unit,"(/80(1h#)/)")
         end_input_read=.false.
         do while (.not. end_input_read)

            read(input_unit,"(a100)", iostat=ioerr) string_read
            if(ioerr /= 0) then
               call error_handler("Error appeared when reading INPUT")
               stop
            end if
            if( string_empty(string_read) ) cycle    ! read next string    

               select case( trim(string_read) )
               
               case("[task]")
                  task_defined=.true.
                  read(input_unit,*) task_key
                  make_crystal_fragment=trim(task_key)=="make_crystal_fragment"
                  make_ewald_array     =trim(task_key)=="make_ewald_array"
                  
               case("[lattice_vectors]")
                  if(.not.task_defined) call error_handler("Input error [0]: Task is not first !")
                  read(input_unit,*) units
                  if(trim(units)=="a.u." .or. trim(units)=="angstrem" ) then
                     read(input_unit,*) (a(:,i_vector),i_vector=1,3)
                     format="(/10X, 'Lattice vectors (',a,')')"
                     write(output_unit,format) trim(units)
                     ...
                     if(gxcv_file_exist) then
                        write (output_unit,*) '=============================================='
                        write (output_unit,*) 'Unit cell has been taken from GULPp'
                        read(gxc_unit,*) (a(:,i_vector),i_vector=1,3)
                        write (output_unit,*) '     New cell vectors'
                        do i_vector=1,3
                           write(output_unit,format) i_vector, a(:,i_vector)
                        end do
                        write(output_unit,*) '----------------------------------------------'
                     endif
                     if(units=="angstrem") then
                        a=a/0.529177_r8_kind
                     else if(units=="a.u.") then
                        a=a
                     end if
...
!                              Reciprocal lattice vectors
			...
			V_cell=abs( V_cell )
                     ...
                     call error_handler("Input error [1]: <units> not specified")
                  end if
         
               case("[center_screep]")
                  read(input_unit,*) Rc_screep(:)
               ...
               case("[atom_positions]")
                  if(.not.task_defined) call error_handler("Input error [0]: Task is not first !")
                  read(input_unit,*) units 
                  if(trim(units)/="[n_atoms_in_cell]") &
                       call error_handler("Input error [2]: no <[n_atoms_in_cell]> keyword")
                  read(input_unit,*) n_atoms_in_cell
                  format="(/10X, 'Number of atoms in cell : ', i4)"
                  write(output_unit,format) n_atoms_in_cell
                  read(input_unit,*) units  ! lattice, xyz_angstrem, xyz_au
                  format="(/10X, 'Atom positions in cell (input ',a, ')' )"
                  write(output_unit,format) trim(units)//" units"
                  allocate(atom(n_atoms_in_cell), tmp_r(3),stat=alloc_stat)
                  if(alloc_stat.ne.0) &
                       call error_handler("Input error [3]: Allocation failed")
		  ...
		  allocate(type_number(n_atoms_in_cell,n_atoms_in_cell),stat=alloc_stat)
                  type_number=.false.
                  if(alloc_stat.ne.0) &
                       call error_handler("Input error [3a]: Allocation failed")
                  do i_atom=1,n_atoms_in_cell
                     read(input_unit,*)  atom(i_atom)%name,tmp_r,atom(i_atom)%q
                     select case(trim(units))
                        case("lattice")
                           atom(i_atom)%rc=matmul(a,tmp_r-Rc_screep)
                           atom(i_atom)%rl=tmp_r-Rc_screep
                           Rc_screepAU=matmul(a,Rc_screep)/0.529177_r8_kind
                        case("xyz_au")
                           atom(i_atom)%rc=tmp_r-Rc_screep
                           atom(i_atom)%rl=matmul(tmp_r-Rc_screep,b)/(2*pi)
                           Rc_screepAU=Rc_screep
                        case("xyz_angstrem")
                           atom(i_atom)%rc=(tmp_r-Rc_screep)/0.529177_r8_kind
                           atom(i_atom)%rl= &
                                matmul(tmp_r-Rc_screep,b)/(2*pi*0.529177_r8_kind)
                           Rc_screepAU=Rc_screep/0.529177_r8_kind
                     end select
                  end do
                  if(gxc_file_exist.or.gxcv_file_exist) then
                     do i_atom=1,n_atoms_in_cell
                        read(gxc_unit,*) atom(i_atom)%an, tmp_r
                        atom(i_atom)%qt=int(atom(i_atom)%an)
                        type_number(atom(i_atom)%qt,i_atom)=.true.
                        atom(i_atom)%c=(tmp_r-Rc_screep)/0.529177_r8_kind
                        atom(i_atom)%rl= &
                             matmul(tmp_r-Rc_screep,b)/(2*pi*0.529177_r8_kind)
                     end do
                     do 
                        read(gxc_unit,*,end=22) an,tmp_r
                        if(an.eq. zero) exit
                        !		 ian=an
                        do i_atom=1,n_atoms_in_cell 
                           if(type_number(int(an),i_atom)) then
                              atom(i_atom)%s=(tmp_r-Rc_screep)/0.529177_r8_kind
...
22                   continue
                     write (output_unit,*)'=============================================='
                     write (output_unit,*) 'Unit cell has been taken from GULP'
                     write (output_unit,*)     '   New coordinates (au)'
                     format="(3x,i4, 3f20.7)"
                     do i_atom=1,n_atoms_in_cell
                        ...
                        atom(i_atom)%rl= &
                             matmul(atom(i_atom)%rc,b)/(2*pi)
                        write(output_unit,format) atom(i_atom)%qt, atom(i_atom)%rc
                     enddo
                     write(output_unit,*) '=============================================='
                  endif

                  !!!!!!!!!!!!!!!!!!!!!!!!
!!$                  allocate(perm(n_atoms_in_cell,3))
!!$                  perm=zero
!!$                  allocate(r_buf(3,n_atoms_in_cell))
!!$                  r_buf=zero
!!$                  axis=(/zero,zero,one/)
!!$                  ang=(/zero, two*pi/three, -two*pi/three/)
!!$                  do i=1,n_atoms_in_cell
!!$                     pos=atom(i)%rc
!!$                     do j=1,3
!!$                        call rot(u,axis,ang(j))
!!$                        pos2=matmul(u,pos)
!!$                        do k=1,n_atoms_in_cell
!!$                           if(dot_product(pos2-atom(k)%rc,pos2-atom(k)%rc).lt.0.9_r8_kind) then
!!$                              perm(i,j)=k
!!$                              exit
!!$                           endif
!!$                        enddo
...
!!$                        call rot(u,axis,ang(j))
!!$                        pos2=matmul(u,pos)
!!$                        r_buf(:,perm(i,j))=r_buf(:,perm(i,j))+pos2
!!$                     enddo
!!$                  enddo
!!$                  atom(:n_atoms_in_cell)%rc(1)=r_buf(1,:n_atoms_in_cell)/3.0_r8_kind
!!$                  atom(:n_atoms_in_cell)%rc(2)=r_buf(2,:n_atoms_in_cell)/3.0_r8_kind
!!$                  atom(:n_atoms_in_cell)%rc(3)=r_buf(3,:n_atoms_in_cell)/3.0_r8_kind
!!$                  
!!$                  deallocate(perm,r_buf)
!!$
!!$                  do i_atom=1,n_atoms_in_cell
!!$                     write(output_unit,format) atom(i_atom)%qt, atom(i_atom)%rc
!!$                     atom(i_atom)%rl=matmul(atom(i_atom)%rc,b)/(2*pi)
!!$                  enddo
                  !!!!!!!!!!!!!!!!!!!!!!!!
		
                  if(make_ewald_array) then 
                     read(input_unit,*) cluster_key
                     if(trim(cluster_key)/="[n_cluster_atoms]") &
                          call error_handler("Input error [4]: no [n_cluster_atoms] keyword")
                     read(input_unit,*) n_cluster_atoms
                     format="(/10X, 'Number of cluster atoms : ', i4)"
                     allocate(cluster(n_cluster_atoms), stat=alloc_stat)
                     if(alloc_stat.ne.0) &
                          call error_handler("Input error [5]: Allocation failed")
                     do i_atom=1,n_cluster_atoms
                        read(input_unit,*)  cluster(i_atom)%name,tmp_r
                        select case(trim(units))
                        case("lattice")
                           cluster(i_atom)%rc=matmul(a,tmp_r-Rc_screep)
                           cluster(i_atom)%rl=tmp_r-Rc_screep
                        case("xyz_au")
                           cluster(i_atom)%rc=tmp_r-Rc_screep
                           cluster(i_atom)%rl=matmul(tmp_r-Rc_screep,b)/(2*pi)
                        case("xyz_angstrem")
                           cluster(i_atom)%rc=(tmp_r-Rc_screep)/0.529177_r8_kind
                           cluster(i_atom)%rl=matmul(tmp_r-Rc_screep,b)/(2*pi*0.529177_r8_kind)
                     end select
                  end do
                  do i_atom=1,n_cluster_atoms
                  ...
                  if(alloc_stat.ne.0) &
                       call error_handler("Input error [6]: Deallocation failed")
                  end if ! make_ewald_array
                  format="(/30X,'X',17x,'Y',20x,'Z',17x,'Q' )"
                  write(output_unit,format)
                  do i_atom=1,n_atoms_in_cell
                  ...
                  format="(/30X,'a_1',15x,'a_2',17x,'a_3',15x,'Q')"
                  write(output_unit,format)
                  do i_atom=1,n_atoms_in_cell
                     format="(7X,i4,4x,a,3x,3(f17.12,3x),3x,f7.3 )"
...
!                                       Cluster output
                  if(make_ewald_array) then
                  write(output_unit,&
                      "(/10X,'Cluster atoms(input ',a,')')") trim(units)//" units"
                  format="(/30X,'X',17x,'Y',20x,'Z',17x,'Q' )"
                  write(output_unit,format)
                  do i_atom=1,n_cluster_atoms
                  ...
                  format="(/30X,'a_1',15x,'a_2',17x,'a_3')"
                  write(output_unit,format)
                  do i_atom=1,n_cluster_atoms
                     format="(7X,i4,4x,a,3x,3(f17.12,3x) )"
               ...
               case("[pc_array]")                  
                  if(.not.task_defined) call error_handler("Input error [0]: Task is not first !")
                  read(input_unit,*) units
                  if(trim(units)/="a.u." .and. trim(units)/="angstrem") &
                       call error_handler("Input error : pc_array <units> not specified")
                  read(input_unit,*) pc_key, R_max_pc
                  if(pc_key/="R_max_pc") &
                       call error_handler("Input error : R_max_pc not found")
                  if(units=="angstrem") R_max_pc = R_max_pc/0.529177_r8_kind
                  format="( 10X/' PC-array size : ', f15.10, ' (a.u.)' )"
                  write(output_unit,format) R_max_pc

               case("[truncate_crystal_fragment]")                  
                  if(.not.task_defined) call error_handler("Input error [0]: Task is not first !")
                  read(input_unit,*) units
                  if(trim(units)/="a.u." .and. trim(units)/="angstrem") &
                       call error_handler("Input error : Truncate fragment <units> not specified")
                  read(input_unit,*) pc_key, R_max_fragment
                  if(pc_key/="R_max_fragment") &
                       call error_handler("Input error : R_max_fragment not found")
                  if(units=="angstrem") R_max_fragment = R_max_fragment/0.529177_r8_kind
                  format="( 10X/' Crystal fragment size : ', f15.10, 1x, A )"
                  write(output_unit,format) R_max_fragment, units

               case("[ewald_parameter]")
                  if(.not.task_defined) call error_handler("Input error [0]: Task is not first !")
                  if(make_crystal_fragment) cycle
                  read(input_unit,*) eta
                  format="(/10X, 'Ewald parameter : ',f10.5)"
                  write(output_unit,format) eta

               case("[r_screep]")
                  if(.not.task_defined) call error_handler("Input error [0]: Task is not first !")
                  if(make_crystal_fragment) cycle
                  read(input_unit,*) r_screep
...
!                  write(output_unit,format) Rc_screep(:)
               ...
               case("[main_axes_order]")
                  if(.not.task_defined) call error_handler("Input error [0]: Task is not first !")
                  if(make_crystal_fragment) cycle
                  read(input_unit,*) axes
			...
			symmetry_c3v=.true.
               ...
               case("[number_of_screep_points]")
                  if(.not.task_defined) call error_handler("Input error [0]: Task is not first !")
                  if(make_crystal_fragment) cycle
                  read(input_unit,*) n_screep_points
               ...
               case("[main_axes_direction]")
                  if(.not.task_defined) call error_handler("Input error [0]: Task is not first !")
                  if(make_crystal_fragment) cycle
                  read(input_unit,*) direction(:)
                  direction(:)=direction(:) / &
                               sqrt(dot_product(direction,direction))
                  format="(/10X,'Orientation(x,y,z) :',3(f10.5,3x))"
                  write(output_unit,format) direction(:)

               case("[screep_procedure]")
                  if(.not.task_defined) call error_handler("Input error [0]: Task is not first !")
                  if(make_crystal_fragment) cycle
                  read(input_unit,*) screep_procedure
                  if(trim(screep_procedure)=="screep_pc" .or. &
                     trim(screep_procedure)=="screep_multipole" .or. &
                     trim(screep_procedure)=="screep_multipole_pc" ) then
                     format="(/10X, 'SCREEP-procedure : ',a)"
                     write(output_unit,format) trim(screep_procedure)
                  else
                     call error_handler("Input error : Wrong screep_procedure")
                  end if
                  if(trim(screep_procedure)=="screep_multipole" .or. &
                     trim(screep_procedure)=="screep_multipole_pc" ) &
                     read(input_unit,*) i_scheme, lmax
                  write(output_unit,"(10X,'Spherical Integration scheme = ',i2)") i_scheme

               case("[end_input]")
                  end_input_read=.true.
                  if(.not.task_defined) &
                       call error_handler("Input error [0]: Task is not defined !")
                  write(output_unit,"(/80(1h#)/)")
                  exit
               end select
         end do 
!                                       Analize input_string
       contains
!
       logical function string_empty(string)
         character(len=*) ::  string
         integer       first_symbol
         first_symbol=0
         temp_string=" "
         do i=1,100
         ...
         if(first_symbol.eq.0) then
            ...
            do i=1, len_trim(string)
               temp_string(i:i)=string(first_symbol+i:first_symbol+i)
               if(temp_string(i:i) .eq. "]") return
            end do
         end if
         call error_handler("Incorrect header found")
         stop
       end function string_empty
!  Varian to "select case(case)" MS-PowerStation Fortran
!      logical function string_found(def_string)
!        character(len=*) :: def_string
!        string_found=.true.
!        if( temp_string .ne. def_string) string_found=.false.        
!        return
!      end function string_found
!             
       end subroutine ewald_input
